...
> Однако целью анализа было - понять на какие метрики бизнес должен воздействовать и какого их значения достичь, чтобы выйти на 25% значения маржинальность. Эта задача и была решена построением модели "as is/to be", позволяющей установить характер и степень влияния метрик на маржинальность.
...
>__Таблица Payments__
![pay](/paym.png)

<p> Финальный запрос представляет собой CTE, состоящий их небольших запросов, сформированных следующей логикой.</p>
1. На основе classes были собраны все дни отчетного периода (2016 года), когда студенты были активны (в таблице есть данные о завершенном действии, опираясь на столбец с датой окончания активности), на основе payments - даты первой оплаты для каждого студента. 
<p> Далее эти CTE были объединены по неравенству для получения датасета об активностях студентов, которые представляют коммерческий интерес для компании (активности после оплаты). </p>
